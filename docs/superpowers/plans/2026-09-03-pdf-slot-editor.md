...
> **Completed; superseded on the fontkit engine.** This task originally ran against `@pdf-lib/fontkit` 1.1.1. A follow-up ruling (R-8) found that engine crashes on `doc.save()` with `{ subset: true }` and switched the project to upstream `fontkit` 2.0.4 — see Task 3's report and the Tech Stack line above. The kerning finding (no GPOS kerning, in measurement or output) was re-verified against `fontkit` 2.0.4 and is unchanged, and is now also pinned by a test that inspects the emitted content stream, not just `widthOfTextAtSize`. Code samples below (and in Tasks 5 and 9 — Task 10 has no `fontkit` import and was not touched) are updated to `import * as fontkit from 'fontkit'` — that package's ESM build has no default export, so `import fontkit from 'fontkit'` throws. `fontkit` ships no `.d.ts`; without `@types/fontkit` 2.0.9 (added as a devDependency of `@pdf-slot/core`, alongside a `typecheck` script — run `npm run typecheck` from the repo root) that import is implicit `any` and nothing checks it against `@cantoo/pdf-lib`'s structural `Fontkit` interface. With `@types/fontkit` installed, the match is compiler-verified.
